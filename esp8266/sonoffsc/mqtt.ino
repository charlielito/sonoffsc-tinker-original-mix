...
void mqttConnect() {
    ...
    if (!mqtt.connected()) {
        ...
        char * user = strdup(getSetting("mqttUser",MQTT_USER).c_str());
        char * pass = strdup(getSetting("mqttPassword",MQTT_PASSWORD).c_str());

        DEBUG_MSG("[MQTT] Connecting to broker at %s user: %s, pass: %s", host, user, pass );
